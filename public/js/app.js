...
function renderCount(filteredOmbres) {
  ...
  const totalCount = filteredOmbres.reduce((sum, o) => sum + (parseInt(o.nombre) || 1), 0);
  if (crF && CR_LIMITS[crF] !== undefined) {
    countDiv.textContent = `${totalCount} / ${CR_LIMITS[crF]} ombres dans CR ${crF}`;
  } else {
    const fullTotal = ombres.reduce((sum, o) => sum + (parseInt(o.nombre) || 1), 0);
...
function renderList() {
  ...
  const crF = (document.getElementById('cr-filter').value || '').trim();
  const tagF = (document.getElementById('tag-filter').value || '').trim().toLowerCase();

  let filtered = ombres.filter(o => {
    let ok = true;
    if (searchQ && !(o.name.toLowerCase().includes(searchQ) || o.tags.join(' ').toLowerCase().includes(searchQ) || o.description.toLowerCase().includes(searchQ))) ok = false;
  ...
  if (filtered.length === 0) {
    ...
    filtered.forEach(o => {
      ...
      html += `<div class="creature-card">
        <h3>${o.name} <span style="font-size:0.90em;font-weight:400;">(CR ${o.cr})</span>
            <span style="margin-left:16px;color:var(--primary-accent);font-weight:600;">x${o.nombre || 1}</span>
        </h3>
        ...
        ${o.statLink ? `<div class="creature-link"><a href="${o.statLink}" target="_blank">Stat Sheet</a></div>` : ""}
        <div style="margin-top:1em;display:flex;gap:0.6em;justify-content:center;">
          ...
          <button class="mini-btn delete-btn" data-id="${o.id}" style="background:#611;color:white;">Supprimer</button>
          <button class="mini-btn plus-btn" data-id="${o.id}" style="background:var(--primary-accent);color:white;">+1</button>
          <button class="mini-btn minus-btn" data-id="${o.id}" style="background:#257;color:white;">-1</button>
        </div>
      </div>`;
    });
  }
  ...
  main.querySelectorAll('.delete-btn').forEach(btn =>
    btn.onclick = () => handleDelete(parseInt(btn.getAttribute('data-id')))
  );
  main.querySelectorAll('.plus-btn').forEach(btn =>
    btn.onclick = () => handleIncrement(parseInt(btn.getAttribute('data-id')))
  );
  main.querySelectorAll('.minus-btn').forEach(btn =>
    btn.onclick = () => handleDecrement(parseInt(btn.getAttribute('data-id')))
  );
}
...
function updateFilters() {
  ...
  tagSel.innerHTML = '<option value="">Tous les tags</option>' + tags.map(t => `<option value="${t}">${t}</option>`).join('');
...
function renderAddForm(isEdit = false, values = {}) {
  ...
  main.innerHTML = `
    <form id="add-form">
      ...
      <label>CR
        <input type="number" name="cr" min="0" required value="${values.cr || ''}">
      </label>
      <label>Tags (séparés par des virgules)
        <input type="text" name="tags" placeholder="tank, volant…" value="${values.tags ? values.tags.join(', ') : ''}">
      </label>
      <label>Description
        <textarea name="description" rows="2" style="font-size:0.97em;">${values.description || ''}</textarea>
      </label>
      <label>Lien stat sheet
        <input type="url" name="statLink" placeholder="https://..." value="${values.statLink || ''}">
      </label>
      <label>Nombre d'ombres identiques
        <input type="number" name="nombre" min="1" required value="${values.nombre || 1}">
      </label>
  ...
  document.getElementById('add-form').onsubmit = async function(e) {
    ...
    if (CR_LIMITS[crCat] !== undefined) {
      const countSansCelleCi = ombres.filter(o => getCRRange(o.cr) === crCat && (!isEdit || o.id !== editId)).reduce((sum, o) => sum + (parseInt(o.nombre) || 1), 0);
      if ((countSansCelleCi + nombre) > CR_LIMITS[crCat]) {
        alert(`Limite atteinte pour la catégorie CR "${crCat}" (${CR_LIMITS[crCat]} max).`);
        return;
      }
    }
    ...
    if (isEdit) {
      await updateOmbre(editId, { name, cr, tags, description, statLink, nombre });
    } else {
...
async function handleIncrement(id) {
  ...
  const countSansCelleCi = ombres.filter(o => getCRRange(o.cr) === crCat && o.id !== id).reduce((sum, o) => sum + (parseInt(o.nombre) || 1), 0);
